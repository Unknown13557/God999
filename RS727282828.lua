...
local slot8 = Slots[8]
local frame = slot8.Frame
slot8.Frame.ZIndex = 20
...
row8.Size = UDim2.new(1, -12, 1, -8)
row8.Position = UDim2.fromOffset(4, 4)
row8.ZIndex = 21
row8.ClipsDescendants = false

local layout8 = Instance.new("UIListLayout")
layout8.Parent = row8
layout8.FillDirection = Enum.FillDirection.Horizontal
layout8.VerticalAlignment = Enum.VerticalAlignment.Center
layout8.HorizontalAlignment = Enum.HorizontalAlignment.Left
layout8.Padding = UDim.new(0, 2)
...
local function readSlot8Config()
...
local function startEscape()
	...
	local targetY, speed = readSlot8Config()
...
local slot14Data = Slots[14]
local slot14Frame = slot14Data.Frame

slot14Frame.AutomaticSize = Enum.AutomaticSize.None
slot14Frame.ClipsDescendants = true

slot14Data.Pill.Visible = false
slot14Data.State = nil
slot14Data.Label.Visible = false

local row14 = Instance.new("Frame")
row14.Parent = slot14Frame
row14.Size = UDim2.new(1, -12, 1, -8)
row14.Position = UDim2.fromOffset(4, 4)
row14.BackgroundTransparency = 1
row14.ZIndex = 20

local row14Layout = Instance.new("UIListLayout")
row14Layout.Parent = row14
row14Layout.FillDirection = Enum.FillDirection.Horizontal
row14Layout.VerticalAlignment = Enum.VerticalAlignment.Center
row14Layout.Padding = UDim.new(0, 2)

local SLOT14MIN_TWEEN_Y = 30
local SLOT14MAX_TWEEN_Y = 20000000

local slot14Input = Instance.new("TextBox")
slot14Input.Parent = row14

slot14Input.Size = UDim2.fromOffset(110, 28)

slot14Input.Text = "500000"
slot14Input.PlaceholderText = "Input Y"
slot14Input.TextColor3 = Color3.fromRGB(230,230,230)
slot14Input.PlaceholderColor3 = Color3.fromRGB(160,160,160)
slot14Input.BackgroundColor3 = Color3.fromRGB(40,40,40)
slot14Input.ClearTextOnFocus = false
slot14Input.Font = Enum.Font.SourceSansBold
slot14Input.TextSize = 16
slot14Input.TextXAlignment = Enum.TextXAlignment.Center
slot14Input.TextYAlignment = Enum.TextYAlignment.Center
slot14Input.ZIndex = 15

local slot14InputCorner = Instance.new("UICorner")
slot14InputCorner.CornerRadius = UDim.new(0,6)
slot14InputCorner.Parent = slot14Input

slot14Input:GetPropertyChangedSignal("Text"):Connect(function()
	local text = slot14Input.Text
	text = text:gsub("%D", "")
	if text == "" then
		slot14Input.Text = ""
		return
	...
	if num > SLOT14MAX_TWEEN_Y then
		num = SLOT14MAX_TWEEN_Y
	end

	local fixed = tostring(math.floor(num))
	if slot14Input.Text ~= fixed then
		slot14Input.Text = fixed
	end
end)

local slot14UpBtn = Instance.new("TextButton")
slot14UpBtn.Parent = row14
slot14UpBtn.Size = UDim2.fromOffset(42, 28)
slot14UpBtn.Text = ""
slot14UpBtn.BackgroundColor3 = Color3.fromRGB(80,180,120)
slot14UpBtn.ZIndex = 15

local slot14UpIcon = Instance.new("ImageLabel")
slot14UpIcon.Parent = slot14UpBtn
slot14UpIcon.BackgroundTransparency = 1
slot14UpIcon.Size = UDim2.fromScale(0.85, 0.85)
slot14UpIcon.Position = UDim2.fromScale(0.075, 0.075)
slot14UpIcon.Image = "rbxassetid://6031090990"
slot14UpIcon.ZIndex = slot14UpBtn.ZIndex + 1

local slot14UpCorner = Instance.new("UICorner")
slot14UpCorner.CornerRadius = UDim.new(0,6)
slot14UpCorner.Parent = slot14UpBtn

local slot14DownBtn = Instance.new("TextButton")
slot14DownBtn.Parent = row14
slot14DownBtn.Size = UDim2.fromOffset(42, 28)
slot14DownBtn.Text = ""
slot14DownBtn.BackgroundColor3 = Color3.fromRGB(180,80,80)
slot14DownBtn.ZIndex = 15

local slot14DownIcon = Instance.new("ImageLabel")
slot14DownIcon.Parent = slot14DownBtn
slot14DownIcon.BackgroundTransparency = 1
slot14DownIcon.Size = UDim2.fromScale(0.85, 0.85)
slot14DownIcon.Position = UDim2.fromScale(0.075, 0.075)
slot14DownIcon.Image = "rbxassetid://6031090990"
slot14DownIcon.Rotation = 180
slot14DownIcon.ZIndex = slot14DownBtn.ZIndex + 1

local slot14DownCorner = Instance.new("UICorner")
slot14DownCorner.CornerRadius = UDim.new(0,6)
slot14DownCorner.Parent = slot14DownBtn

local function applyOffset(dir)
	local value = tonumber(slot14Input.Text)
	...
	if targetY < SLOT14MIN_TWEEN_Y then
		targetY = SLOT14MIN_TWEEN_Y
end

	if targetY > SLOT14MAX_TWEEN_Y then
		targetY = SLOT14MAX_TWEEN_Y
end
...
slot14UpBtn.MouseButton1Click:Connect(function()
    applyOffset(1)
end)

slot14DownBtn.MouseButton1Click:Connect(function()
    applyOffset(-1)
end)
...
local slot4 = Slots[4]
slot4.Label.Text = "Waypoint"
slot4.State = false
slot4.Frame.ClipsDescendants = true


local row4 = Instance.new("Frame")
row4.Parent = slot4.Frame
row4.Size = UDim2.new(1, -12, 1, -8)
row4.Position = UDim2.fromOffset(4, 4)
row4.BackgroundTransparency = 1
row4.ZIndex = 20

local layout4 = Instance.new("UIListLayout")
layout4.Parent = row4
layout4.FillDirection = Enum.FillDirection.Horizontal
layout4.VerticalAlignment = Enum.VerticalAlignment.Center
...
layout9.HorizontalAlignment = Enum.HorizontalAlignment.Center
layout9.VerticalAlignment = Enum.VerticalAlignment.Center
layout9.Padding = UDim.new(0, 2)
...
slot20.Label.Text = "Virtual Floor"
slot20.State = false
...
local function startLockY()
	...
	lockedY = hrp.Position.Y - 1

	virtualFloor = Instance.new("Part")
	virtualFloor.Size = Vector3.new(30, 1, 30)
...
local slot6 = Slots[6]
slot6.Label.Text = "Infinity Jump"
slot6.State = false
...
local function startInfinityJump()
	...
	infJumpConn = UserInputService.JumpRequest:Connect(function()
		if not slot6.State then return end
...
slot6.Pill.MouseButton1Click:Connect(function()
	slot6.State = not slot6.State

	if slot6.State then
		slot6.Pill.BackgroundColor3 = Color3.fromRGB(120,200,120)
		slot6.SlotKnob:TweenPosition(
			UDim2.fromOffset(20,2),
			Enum.EasingDirection.Out,
			Enum.EasingStyle.Quad,
			0.15,
		...
		slot6.Pill.BackgroundColor3 = Color3.fromRGB(80,80,80)
		slot6.SlotKnob:TweenPosition(
			UDim2.fromOffset(2,2),
			Enum.EasingDirection.Out,
			Enum.EasingStyle.Quad,
...
slot16.Label.Text = "Speed Force"
slot16.State = false
slot16.Frame.ClipsDescendants = true


local row16 = Instance.new("Frame")
row16.Parent = slot16.Frame
row16.Size = UDim2.new(1, -12, 1, -8)
row16.Position = UDim2.fromOffset(120, 4)
row16.BackgroundTransparency = 1
row16.ZIndex = 20

local layout16 = Instance.new("UIListLayout")
layout16.Parent = row16
layout16.FillDirection = Enum.FillDirection.Horizontal
layout16.VerticalAlignment = Enum.VerticalAlignment.Center
layout16.Padding = UDim.new(0, 1)



local speedBox = Instance.new("TextBox")
speedBox.Parent = row16
speedBox.Size = UDim2.fromOffset(40, 28)
speedBox.Text = "300"
...
slot15.Label.Text = "Jump Force"
slot15.State = false
slot15.Frame.ClipsDescendants = true


local row15 = Instance.new("Frame")
row15.Parent = slot15.Frame
row15.Size = UDim2.new(1, -12, 1, -8)
row15.Position = UDim2.fromOffset(120, 4)
row15.BackgroundTransparency = 1
row15.ZIndex = 20

local layout15 = Instance.new("UIListLayout")
layout15.Parent = row15
layout15.FillDirection = Enum.FillDirection.Horizontal
layout15.VerticalAlignment = Enum.VerticalAlignment.Center
layout15.Padding = UDim.new(0, 1)

local jumpBox = Instance.new("TextBox")
jumpBox.Parent = row15
...
local slot1 = Slots[1]
slot1.Label.Text = "Player List"
slot1.State = false
slot1.Frame.ClipsDescendants = false
slot1.Pill.Visible = false
slot1.SlotKnob.Visible = false

slot1.Label.Size = UDim2.new(1, -30, 1, 0)
slot1.Label.TextXAlignment = Enum.TextXAlignment.Left

local arrowIcon = Instance.new("ImageLabel")
arrowIcon.Parent = slot1.Frame
...
local PlayerListFrameStroke = Instance.new("UIStroke")
PlayerListFrameStroke.Parent = PlayerListFrame
PlayerListFrameStroke.Color = Color3.fromRGB(255, 255, 255)
PlayerListFrameStroke.Thickness = 1.5
PlayerListFrameStroke.Transparency = 0
PlayerListFrameStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
...
slot10.Label.Text = "Teleport Player"
slot10.State = false
...
row10.Position = UDim2.fromOffset(120, 4)
...
local function createESP(player)
	...
	local function attachCharacter(char)
		...
		conn = RunService.RenderStepped:Connect(function()
			...
			if not slot2.State then return end
...
slot12.Label.Text = "TP Player Orbit"
slot12.State = false
...
row12.Position = UDim2.fromOffset(120, 4)
...
radiusStrafeBox.Parent = row12
radiusStrafeBox.Size = UDim2.fromOffset(40, 28)
radiusStrafeBox.Text = "990"
...
slot11.Label.Text = "TP Backstab"
slot11.State = false
slot11.Frame.ClipsDescendants = true

local row11 = Instance.new("Frame")
row11.Parent = slot11.Frame
row11.Size = UDim2.new(1, -12, 1, -8)
row11.Position = UDim2.fromOffset(120, 4)
row11.BackgroundTransparency = 1

local layout11 = Instance.new("UIListLayout")
layout11.Parent = row11
layout11.FillDirection = Enum.FillDirection.Horizontal
layout11.VerticalAlignment = Enum.VerticalAlignment.Center
layout11.Padding = UDim.new(0, 1)

local radiusBackstabBox = Instance.new("TextBox")
radiusBackstabBox.Parent = row11
...
slot11.Pill.MouseButton1Click:Connect(function()

	slot11.State = not slot11.State

	if slot11.State then

		slot11.Pill.BackgroundColor3 = Color3.fromRGB(120,200,120)
		slot11.SlotKnob:TweenPosition(
			UDim2.fromOffset(20,2),
			Enum.EasingDirection.Out,
			Enum.EasingStyle.Quad,
			0.15,
      ...
      addNoclipUser()
		...
		slot11.Pill.BackgroundColor3 = Color3.fromRGB(80,80,80)
		slot11.SlotKnob:TweenPosition(
			UDim2.fromOffset(2,2),
			Enum.EasingDirection.Out,
			Enum.EasingStyle.Quad,
...
local slot3 = Slots[3]
slot3.Label.Text = "Cam Lock ( Nearest )"
slot3.State = false
...
slot3.Pill.MouseButton1Click:Connect(function()

	slot3.State = not slot3.State

	if slot3.State then

		slot3.Pill.BackgroundColor3 = Color3.fromRGB(120,200,120)
		slot3.SlotKnob:TweenPosition(
			UDim2.fromOffset(20,2),
			Enum.EasingDirection.Out,
			Enum.EasingStyle.Quad,
			0.15,
		...
		slot3.Pill.BackgroundColor3 = Color3.fromRGB(80,80,80)
		slot3.SlotKnob:TweenPosition(
			UDim2.fromOffset(2,2),
			Enum.EasingDirection.Out,
			Enum.EasingStyle.Quad,
...
row17.Position = UDim2.fromOffset(120, 4)
...
local function startHitbox()
	...
	hitboxLoop = RunService.Heartbeat:Connect(function(dt)
...
slot13.Label.Text = "Lock Target Y"
slot13.State = false
...
local function startDynamicHeight()
	...
	dynamicFloor.Size = Vector3.new(30, 1, 30)
	...
	dynamicHeightConn = RunService.Stepped:Connect(function()
		...
		if not slot13.State then return end
		if not selectedPlayer then return end
		if not LocalPlayer.Character then return end
		...
		if not targetChar or not myHRP then return end
	...
	dynamicRemoveConn = selectedPlayer.AncestryChanged:Connect(function(_, parent)
		...
		if not parent and slot13.State then
			stopDynamicHeight()
		end
...
local function bindToPlayer(player)
	...
	--------------------------------------------------
	-- Keep your requested remove logic
	--------------------------------------------------
...
local slot5 = Slots[5]
slot5.Label.Text = "Enable Position"
slot5.State = false
...
row5.Position = UDim2.fromOffset(120, 4)
...
local function startPosition()
	...
	positionConn = RunService.RenderStepped:Connect(function()
		...
		if not slot5.State then return end
		if not LocalPlayer.Character then return end
