...
row16.Position = UDim2.fromOffset(4,4)
...
local slot1 = Slots[1]
slot1.Label.Text = "Virtual Floor"
slot1.Frame.ClipsDescendants = true

local virtualFloor = nil
local lockedYValue = 20
...
row1.Parent = slot1.Frame
row1.Size = UDim2.new(1,-12,1,-8)
row1.Position = UDim2.fromOffset(120,4)
...
layout1.Padding = UDim.new(0,1)

local offsetBox = Instance.new("TextBox")
offsetBox.Parent = row1
offsetBox.Size = UDim2.fromOffset(40,28)
offsetBox.Text = tostring(lockedYValue)
...
local slot2 = Slots[2]
slot2.Label.Text = "Hitbox Expand"
...
row2.Position = UDim2.fromOffset(120,4)
...
row8.Parent = slot8.Frame
row8.Size = UDim2.new(1,-12,1,-8)
row8.Position = UDim2.fromOffset(120,4)
...
speedBox:GetPropertyChangedSignal("Text"):Connect(function()
	...
	if speedBox.Text ~= tostring(num) then
...
local function readJumpForce()

	local dt = DEFAULT_JUMP_FORCE

	if Slots[5]
	and Slots[5].State
	and Slots[5].ReadJumpForce then

		dt = Slots[5].ReadJumpForce()
...
BindSlot(slot6,{
	...
	Callback = function(dt,char,hum,hrp)
		...
		local vel = hrp.AssemblyLinearVelocity
		local jumpForce = readJumpForce()

		--------------------------------------------------
		-- HOLD = JETPACK
		--------------------------------------------------

		if jumpHeld or hum.Jump then
		...
		if not jumpRequested then return end
		jumpRequested = false
...
inputInstantTpYBox.Parent = slot3.Frame
...
upInstantTpYBtn.Parent = slot3.Frame
...
downInstantTpYBtn.Parent = slot3.Frame
...
slot14.Label.Text = "Enable Position"  
...
row14.Position = UDim2.fromOffset(120,4)  
...
local function createGui()
	...
	if positionGui then return end
	...
	frame.Size = UDim2.fromOffset(180,50)
	...
	positionLabel.Size = UDim2.new(1,-10,0,28)
	...
	positionLabel.ZIndex = 10000

	speedLabel = Instance.new("TextLabel")
	speedLabel.Parent = frame
	speedLabel.Size = UDim2.new(1,-10,0,28)
	speedLabel.Position = UDim2.fromOffset(5,20)
...
BindSlot(slot14,{
	...
	Callback = function(_,char,hum,hrp)
		...
		if not positionGui or not positionGui.Parent then
			createGui()
		end
		...
		if x ~= lastX or y ~= lastY or z ~= lastZ then  
			...
			positionLabel.Text = "X: "..x.." Y: "..y.." Z: "..z  
			lastPosString = x..", "..y..", "..z  
...
UpdateController:On("ForceDisable",function(name)
	...
	if name ~= "EnablePosition" then return end
...
local slot9 = Slots[9]
slot9.Label.Text = "Player List"
slot9.State = false
slot9.Frame.ClipsDescendants = false
slot9.Pill.Visible = false
slot9.SlotKnob.Visible = false

slot9.Label.Size = UDim2.new(1, -30, 1, 0)
slot9.Label.TextXAlignment = Enum.TextXAlignment.Left
...
--------------------------------------------------
-- SHUTDOWN PATCH FOR SLOT9
--------------------------------------------------

local function ShutdownSlot9()
...
BindSlot(slot10,{
	...
	Callback = function(dt,char,hum,hrp)
		...
		if not slot10.State then
			if removeNoclipUser then
				removeNoclipUser()
			end
			return
		end
...
local function forceBackstabMove(hrp,targetHRP,speed,radius,dt)

	local myPos = hrp.Position

	local behindPos =
		(targetHRP.CFrame * CFrame.new(0,0,radius)).Position

	-- lock Y với target để tránh rơi
	behindPos = Vector3.new(
		behindPos.X,
		targetHRP.Position.Y + 0.1,
		behindPos.Z
	)

	local delta = behindPos - myPos
	local dist = delta.Magnitude
...
row7.Parent = slot7.Frame
row7.Size = UDim2.new(1,-12,1,-8)
row7.Position = UDim2.fromOffset(4,4)
row7.BackgroundTransparency = 1
row7.ZIndex = 20

local layout7 = Instance.new("UIListLayout")
layout7.Parent = row7
layout7.FillDirection = Enum.FillDirection.Horizontal
layout7.VerticalAlignment = Enum.VerticalAlignment.Center
layout7.Padding = UDim.new(0,6)
...
slot12.Label.Text = "Lock Target Y"
...
slot13.Label.Text = "Spectator Player"
...
local MAX_TP_STEP = 450
local CLUSTER_RADIUS = 120
...
row.Parent = slot4.Frame
row.Size = UDim2.new(1,-12,1,-8)
row.Position = UDim2.fromOffset(120,4)
...
auraBox.Parent = row
auraBox.Size = UDim2.fromOffset(40,28)
auraBox.Text = tostring(auraRadius)
auraBox.PlaceholderText = "Aura"
auraBox.ClearTextOnFocus = false
auraBox.Font = Enum.Font.SourceSansBold
auraBox.TextSize = 14
auraBox.TextXAlignment = Enum.TextXAlignment.Center
auraBox.TextColor3 = Color3.fromRGB(200,200,200)
auraBox.BackgroundColor3 = Color3.fromRGB(40,40,40)
...
auraBox:GetPropertyChangedSignal("Text"):Connect(function()

    local filtered = auraBox.Text:gsub("%D", "")
...
local function refreshPlayers()
    ...
    -- Clear only if necessary (When players join/leave)
...
-- If aura parts are already created, don't recreate them
local function createAura()

    if #auraParts > 0 then return end  -- Avoid recreating if already created
...
local function updateAura(hrp)
    ...
    -- Only update if there is a change in position or size
...
local function predictEnemy(enemyHRP)
    local vel = enemyHRP.AssemblyLinearVelocity
    return enemyHRP.Position + vel * 0.25
end
...
local function scanEnemies(hrp)

    local enemies = {}
    local scanRange = auraRadius + SCAN_OFFSET
    local pos = hrp.Position

    -- Only scan players who are within the scan range
    for _, p in ipairs(cachedPlayers) do
        if not p or not p.Character then continue end  -- Skip invalid players

        local char = p.Character
        local e = char:FindFirstChild("HumanoidRootPart")
        if not e then continue end

        -- Add enemies if they are within scan range
        if (e.Position - pos).Magnitude <= scanRange then
...
local function detectCluster(enemies)
    ...
    for i = 1, n do
        ...
        for j = 1, n do
            if i ~= j then
                ...
                if (a.Position - b.Position).Magnitude < CLUSTER_RADIUS then
                    count += 1
                end
            end
        end
...
local function surroundScore(hrp, enemies)
    ...
    for _, e in ipairs(enemies) do
        ...
        local diff = e.Position - pos
        if diff.Magnitude < 0.1 then continue end

        local dir = diff.Unit

        if dir:Dot(rightv) > 0.5 then
            right = true
        elseif dir:Dot(rightv) < -0.5 then
            left = true
        end

        if dir:Dot(look) > 0.5 then
            front = true
        elseif dir:Dot(look) < -0.5 then
            back = true
        end
    end

    if left then score += 1 end
    if right then score += 1 end
    if front then score += 1 end
    if back then score += 1 end
...
local function scorePos(pos, enemies)
...
local function rayBlocked(origin, pos, offset)
...
local function pathBlocked(origin, pos, hrp)

    local right = hrp.CFrame.RightVector * 2

    if rayBlocked(origin, pos, Vector3.zero) then return true end
    if rayBlocked(origin, pos, right) then return true end
    if rayBlocked(origin, pos, -right) then return true end
...
local function findEscape(hrp, enemies)
    ...
    local desiredDistance = auraRadius + 20
    local escapeDistance = math.min(desiredDistance, MAX_TP_STEP)
    ...
    for _, dir in ipairs(directions) do
        ...
        if pathBlocked(origin, pos, hrp) then continue end
        if spaceBlocked(pos) then continue end

        local score = scorePos(pos, enemies)
...
local function enemyThreat(hrp, enemies)

    local pos = hrp.Position

    for _, e in ipairs(enemies) do

        local predicted = predictEnemy(e)

        if (predicted - pos).Magnitude < auraRadius then
            return true
        end
    end
...
local function startForceField()
    ...
    dodgeConn = RunService.Heartbeat:Connect(function()
        ...
        local enemies = scanEnemies(hrp)
        if #enemies == 0 then return end

        local surround = surroundScore(hrp, enemies)
        local cluster = detectCluster(enemies)

        if enemyThreat(hrp, enemies) or surround >= 3 or cluster then
...
_G.StopForceFieldSlot4 = function()
...
local function setToggle(state)    
    ...
    if state then    
        ...
        slot15.SlotKnob.Position = UDim2.fromOffset(20,2)    
        enableESP()    
    else    
        slot15.Pill.BackgroundColor3 = Color3.fromRGB(80,80,80)    
        slot15.SlotKnob.Position = UDim2.fromOffset(2,2)    
        disableESP()    
    end    
end    
    
slot15.Pill.MouseButton1Click:Connect(function()    
    setToggle(not slot15.State)    
end)    
...
_G.StopESPPlayerSlot15 = function()    
    if espLoopConn then    
        espLoopConn:Disconnect()    
        espLoopConn = nil    
    end    
...
slot17.Label.Text = "System"
slot17.State = false
slot17.Frame.ClipsDescendants = true
slot17.Pill.Visible = false
...
row17.Position = UDim2.fromOffset(4,4)
...
layout17.HorizontalAlignment = Enum.HorizontalAlignment.Center
layout17.VerticalAlignment = Enum.VerticalAlignment.Center
layout17.Padding = UDim.new(0,2)
...
--------------------------------------------------
	...
	if typeof(updateSharedNoclip) == "function" then
		updateSharedNoclip()
	end
...
if _G.StopForceFieldSlot4 then
	pcall(_G.StopForceFieldSlot4)
	_G.StopForceFieldSlot4 = nil
end


if _G.StopESPPlayerSlot15 then
	pcall(_G.StopESPPlayerSlot15)
end
