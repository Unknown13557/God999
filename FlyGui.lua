...
BindSlot(Waypoint,{
	...
	Callback = function(dt,char,hum,hrp)
		...
		-- HP CHECK (thêm duy nhất phần này)
		waypointEnabled = updateHPState(waypointEnabled)
...
local teleportFreqBox = Instance.new("TextBox")
teleportFreqBox.Parent = rowTPInstant
teleportFreqBox.Size = UDim2.fromOffset(40,28)
teleportFreqBox.Text = "30"
teleportFreqBox.PlaceholderText = "Hz"
teleportFreqBox.ClearTextOnFocus = false
teleportFreqBox.Font = Enum.Font.SourceSansBold
teleportFreqBox.TextSize = 14
teleportFreqBox.TextXAlignment = Enum.TextXAlignment.Center
teleportFreqBox.TextColor3 = Color3.fromRGB(200,200,200)
teleportFreqBox.BackgroundColor3 = Color3.fromRGB(40,40,40)
teleportFreqBox.BorderSizePixel = 0
teleportFreqBox.ZIndex = 21
Instance.new("UICorner",teleportFreqBox).CornerRadius = UDim.new(0,6)
...
local teleportFreq = 30
local teleportInterval = 1 / teleportFreq
local accumulator = 0

teleportFreqBox:GetPropertyChangedSignal("Text"):Connect(function()

	local filtered = teleportFreqBox.Text:gsub("%D","")

	if filtered == "" then
		teleportFreqBox.Text = ""
		return
	end
	...
	teleportFreq = num
	teleportInterval = 1 / teleportFreq

	teleportFreqBox.Text = tostring(num)
...
local teleportInstantEnabled = true

--------------------------------------------------
-- BIND SLOT
--------------------------------------------------

BindSlot(TeleportPlayerInstant,{

	Name = "TeleportPlayerInstant",
	Type = "HeartbeatOverride",
	Priority = 8,
	Default = false,
	...
	Callback = function(dt,char,hum,hrp)
		...
		if not TeleportPlayerInstant.State then
			if removeNoclipUser then
				removeNoclipUser()
			end
			return
		...
		teleportInstantEnabled =
			updateHPState(teleportInstantEnabled)

		if not teleportInstantEnabled then
			return
		end
		...
		if accumulator < teleportInterval then
			return
		end
...
UpdateController:On("ForceDisable",function(name)
	...
	if name == "TeleportPlayerInstant" then
