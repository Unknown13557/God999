...
-- overlay button để click toàn bộ slot
...
local function removePlayer(plr)
	...
	if playerButtons[plr] then
		...
		-- nếu player bị remove đang được chọn
...
local function updatePlayerListPosition()
    ...
    -- nếu bị âm thì đặt sang bên phải DragFrame
    if x < 0 then
...
local MAX_TELEPORT_PLAYER_SPEED = 1000
...
local function startTargetStrafe()
	...
	strafeConnection = RunService.Stepped:Connect(function(_, dt)
		...
		local speed = readTeleportSpeed() -- dùng chung speedbox
		...
		-- Nếu còn xa hơn radius → bay tới target
		if distance > radius then
			...
			-- Đã vào phạm vi → bắt đầu quay vòng
