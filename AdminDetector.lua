...
local function attachCharacterForHP(p, char)
	...
	if hum then
		connectHumanoidForHP(p, hum)
	else
		log("Không thấy Humanoid của", p.Name, "sau timeout")
	end
end
...
-- ========== ÁP DỤNG ==========
-- 1) Gắn cho tất cả người đang trong server
...
-- 3) Quét lại toàn bộ sau khi mọi thứ load đủ (1 giây)
task.defer(function()
	task.wait(1)
	for _, p in ipairs(Players:GetPlayers()) do
...
if DIAGNOSTIC then
	task.defer(function()
		log("Khởi động | HP_THRESHOLD =", HP_THRESHOLD, "| Blocked =", (function()
			local t = {}
			for k in pairs(BLOCKED_USERNAMES) do table.insert(t, k) end
			table.sort(t)
